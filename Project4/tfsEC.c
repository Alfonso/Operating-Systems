...
int dir_find(uint16_t ino, const char *fname, size_t name_len, struct dirent *dirent) {
    ...
    for( indirectPtrIdx = 0; indirectPtrIdx < (sizeof(inode->indirect_ptr) / sizeof(int)); indirectPtrIdx++ ){
        ...
        int res = getBlockNums( indirectPtrIdx, blockArr, inode );
        if( res > 0 ){
...
// this function gets an array of data block
// nums that are found in the given indirect block
int getBlockNums(int indirectPtrIdx, int* numArr, struct inode* inode){
